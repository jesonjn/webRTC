...
public class IceClient {
	...
	private String[] turnServers = new String[] { "114.215.81.102:3478" };

	private String[] stunServers = new String[] { "114.215.81.102:3478" };

	private String username = "100";

	private String password = "100pass";
	...
	public SocketAddress getRemotePeerSocketAddress() {
		...
		Log.i("tag",
				"Remote candinate transport address:"
						+ remoteCandidate.getTransportAddress());
		Log.i("tag",
				"Remote candinate host address:"
						+ remoteCandidate.getHostAddress());
		Log.i("tag",
				"Remote candinate mapped address:"
						+ remoteCandidate.getMappedAddress());
		Log.i("tag",
				"Remote candinate relayed address:"
						+ remoteCandidate.getRelayedAddress());
		Log.i("tag",
				"Remote candinate reflexive address:"
	 ...
	 * server(SIP server)
	 */
	public void exchangeSdpWithPeer(String remoteSdp) throws Throwable {
		Log.i("tag", "Paste remote SDP here. Enter an empty line to proceed:");
...
//		remoteSdp = remoteSdp;
	...
	public void startConnect() throws InterruptedException {
		...
		if (TextUtils.isEmpty(remoteSdp)) {
	...
	private IceMediaStream createStream(int rtpPort, String streamName,
		...
		Log.i("tag", "RTP Component created in " + (endTime - startTime)
				+ " ms");
		...
		public void propertyChange(PropertyChangeEvent event) {
			...
			if (state == IceProcessingState.COMPLETED) {
				...
				for (IceMediaStream stream : streams) {
					...
					Log.i("tag",
							"nominated check list:" + stream.getCheckList());
				}
				synchronized (this) {
					this.notifyAll();
				}
